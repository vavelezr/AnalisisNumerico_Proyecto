function [T, respuesta] = ReglaFalsa(func, x0, x1, Tol, niter, Tipo_error)
    ...
    elseif fs(c+1) == 0
        xm(c+1) = xs(c+1); % raíz como el límite superior
        fm(c+1) = eval(subs(f, xm(c+1))); 
        E(c+1) = 0; 
        respuesta = sprintf('El límite superior %f es una raíz exacta de f(x)', xs(c+1)); 
        T = table((0:1:c)', xm', xi', xs', fm', fi' , fs', E', 'VariableNames', ["n", "x_m", "x_i", "x_s", "f_m", "f_i", "f_s", "E"]); 
    
    % Regla Falsa en donde se encuentra la raíz aprox
    elseif fs(c+1) * fi(c+1) < 0
        fi(c+1) = eval(subs(f, xi)); % val de la funcion en el nuevo límite inferior
        fs(c+1) = eval(subs(f, xs)); % val de la funcion en el nuevo límite superior
        p = (fs(c+1) - fi(c+1)) / (xs(c+1) - xi(c+1)); % pendiente de la secante
        xm(c+1) = (p * xi(c+1) - fi(c+1)) / p; 
        fm(c+1) = eval(subs(f, xm(c+1)));
        fe = fm(c+1); 
        E(c+1) = Tol + 1; % establecer el error inicial como mayor que la tolerancia
        error = E(c+1);



        while error > Tol && fe ~= 0 && c < niter
            if fi(c+1) * fe < 0 % Determinar en qué subintervalo está la raíz
                xs(c+2) = xm(c+1);
            ...
            p = (fs(c+2) - fi(c+2)) / (xs(c+2) - xi(c+2)); % Calcular la pendiente de la recta secante
            xm(c+2) = (p * xi(c+2) - fi(c+2)) / p; % Calcular la nueva aproximación de la raíz
            fm(c+2) = eval(subs(f, xm(c+2))); % Calcular el valor de la función en la nueva aproximación de la raíz
            fe = fm(c+2); 
